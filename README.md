...
`cursor/cli-config.json` tracks portable Cursor CLI preferences, including `approvalMode: auto-review`, Vim mode, display settings, sandbox settings, attribution, and a status line that reuses `~/.claude/scripts/context-bar.sh`. Setup and update shallow-merge those preferences into `~/.cursor/cli-config.json` with `jq` for the `ai` and `full` profiles.

The live file is never symlinked because Cursor also writes authentication, caches, timestamps, and other machine state there. Merge failures leave it untouched, and symlink targets are refused. The command allowlist (`permissions`) is deliberately not tracked or applied, so interactive permissions remain local.
